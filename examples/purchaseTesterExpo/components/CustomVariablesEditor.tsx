...
import { useCustomVariables, customVariablesToStringMap, stringMapToCustomVariables } from './CustomVariablesContext';
...
const CustomVariablesEditor: React.FC = () => {
  const { customVariables, setCustomVariables, showEditor, setShowEditor } = useCustomVariables();
  ...
  useEffect(() => {
    if (showEditor) {
      ...
      setLocalVariables(
        Object.entries(stringMap).map(([key, value]) => ({ key, value }))
      );
      setIsAdding(false);
      setNewKey('');
      setNewValue('');
    }
  }, [showEditor, customVariables]);
  ...
  const handleSave = () => {
    ...
    setCustomVariables(stringMapToCustomVariables(result));
    setShowEditor(false);
  };
